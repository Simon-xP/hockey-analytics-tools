...
All leakage tests hit the real dev database and skip cleanly
if the expected data isn't loaded.
"""
...
def _find_player_with_games_around(cutoff: date) -> int | None:
...
def active_nhl_id() -> int:
    nhl_id = _find_player_with_games_around(CUTOFF)
    if nhl_id is None:
        pytest.skip(
