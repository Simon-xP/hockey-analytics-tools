...
def extract_opponent_features(
    ...
    """Extract opponent team stats as features.

    Computes season-to-date and rolling-10-game opponent averages from
    game_advanced_stats. For the PP model, uses opponent PK stats.
    """
    features = {}

    # For PP model, look at opponent's PK stats (mirror situation)
    opp_situation = situation
    if situation == "pp":
        opp_situation = "pk"
    elif situation == "pk":
        opp_situation = "pp"

    if opp_situation == "other_combined":
        sit_filter = "gas.situation IN ('4v4', '3v3', 'other')"
    else:
        sit_filter = f"gas.situation = '{opp_situation}'"

    rows = session.execute(
        text(f"""
            SELECT gas.toi_seconds, gas.ca, gas.xga, gas.sa, gas.hdca, gas.ga
            FROM game_advanced_stats gas
            JOIN games g ON gas.game_id = g.game_id
            WHERE gas.team_id = :tid AND {sit_filter}
                  AND g.date < :before_date AND gas.toi_seconds > 0
            ORDER BY g.date DESC
            LIMIT 82
        """),
    ...
    # Aggregate team stats (sum across all players, then divide by games)
    # Actually — these are per-player rows. We need team-level stats.
    # Team-level = sum of one player's on-ice stats would overcount.
    # Better approach: use opponent's goals against from the games table.

    # Simpler: use game scores for opponent GAA
    ...
    if game_rows:
        # Opponent goals against = goals the OTHER team scored
        ga_values = []
        for g in game_rows:
            if g[0] == opponent_team_id:
                ga_values.append(g[2])  # away_score = goals against home team
            else:
                ga_values.append(g[1])  # home_score = goals against away team

        if ga_values:
            features["opp_gaa"] = sum(ga_values) / len(ga_values)
            features["opp_gaa_10"] = (
                sum(ga_values[:10]) / min(len(ga_values), 10)
            )
        ...
        for g in game_rows:
            if g[0] == opponent_team_id:
                gf_values.append(g[1])
            else:
                gf_values.append(g[2])

        if gf_values:
            features["opp_gfa"] = sum(gf_values) / len(gf_values)
    ...
    if game_rows:
        most_recent_date = session.execute(
            text("""
                ...
                WHERE (g.home_team_id = :tid OR g.away_team_id = :tid)
                      AND g.date < :before_date AND g.home_score IS NOT NULL
            """),
            {"tid": opponent_team_id, "before_date": before_date},
        ).scalar()

        if most_recent_date:
            days_since = (before_date - most_recent_date).days
...
def extract_all_features(
    ...
    """
    # Determine situation for querying
    query_situation = situation
    if situation == "other":
        query_situation = "other_combined"

    # Load player's prior games in this situation.
    # For PK and Other, use ALL available history (not just current season)
    # because events are rare and we need more data to smooth out noise.
    # For 5v5 and PP, use current season only (sufficient sample size).
    if situation in ("pk", "other"):
        season_start_gid = None  # all history
    else:
