...
from src.optimize.slots import (
    assign_players_to_slots,
    get_teams_playing_on_date,
)
from src.optimize.models import PlayerType, PlayerValue
...
def _default_forecast_fn(
    ...
    *,
    ...
    if not Path("models/forecasting_v2").exists():
        raise RuntimeError("Forecasting models not trained. Run: python -m scripts.train_xg_model")
...
def _resolve_player_context(
    ...
    """Resolve player info, positions, roster context, and historical stats.

    Returns None if the player is a goalie or not found.
    """
...
def _project_games(
    ...
    """Run forecasts for each game where the player can fill an active slot."""
    ...
    for game in games:
        ...
        playing_players = [
            p for p in ctx["working_roster"].players if p.team in teams_playing
        ]

        if can_player_fill_slot(
            ctx["roster_player"], playing_players, roster.roster_slot_settings
        ):
            projected = forecast_fn(nhl_id, game_date, ctx["avg_toi"])
            game_projections[game_date] = projected
            fillable_games += 1

    return game_projections, fillable_games
...
def compute_player_value(
    ...
    if forecast_fn is None:
        forecast_fn = partial(_default_forecast_fn, as_of=as_of)

    ctx = _resolve_player_context(session, nhl_id, roster, season, as_of=as_of)
    if not ctx:
        return None

    week_games = get_team_week_games(session, ctx["team"].abbrev, yahoo_week)
    game_projections, fillable_games = _project_games(
        nhl_id, week_games, ctx, roster, forecast_fn,
    )

    window_fpts = sum(game_projections.values())
    fpts_per_game = window_fpts / fillable_games if fillable_games > 0 else 0.0

    remaining_games = get_team_remaining_games(session, ctx["team"].abbrev, from_date=as_of)
...
def compute_player_value_window(
    ...
    ctx = _resolve_player_context(session, nhl_id, roster, season, as_of=cutoff)
    if not ctx:
        return None

    window_games = get_team_games_in_window(session, ctx["team"].abbrev, start_date, end_date)
    game_projections, fillable_games = _project_games(
        nhl_id, window_games, ctx, roster, forecast_fn,
    )

    window_fpts = sum(game_projections.values())
    fpts_per_game = window_fpts / fillable_games if fillable_games > 0 else 0.0

    remaining_games = get_team_remaining_games(session, ctx["team"].abbrev, from_date=cutoff)
    ros_value = fpts_per_game * remaining_games
...
def find_optimal_window_simple(
    ...
    if not skip_upside:
        ...
        opportunity = compute_opportunity_score(session, nhl_id, as_of=cutoff)
    best_value: Optional[PlayerValue] = None
    best_fpts = -1.0
...
def compute_player_value_simple(
    ...
    if not skip_upside:
        ...
        opportunity = compute_opportunity_score(session, nhl_id, as_of=as_of)
